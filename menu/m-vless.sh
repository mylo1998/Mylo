...
if check_ip_and_get_info "$IP_VPS"; then
    ...
    echo -e "➥ Hubungi admin ${CYAN}「 ✦ @MrFrog125 ✦ 」${NC}"
    exit 1
fi
...
xhttp1="vless://${uuid}@${domain}:443?sni=${domain}&host=${domain}&security=tls&type=xhttp&path=/xvless&encryption=none#${user}"
xhttp2="vless://${uuid}@${domain}:80?path=/xvless&mode=auto&encryption=none&type=xhttp&host=${domain}#${user}"
...
Security     : auto
Network      : WS or XHTTP
Path vless   : <code>/vlessws</code>
XHTTP        : <code>/xvless</code>
◇━━━━━━━━━━━━━━━━━◇
Link TLS     :
<code>${vless1}</code>
◇━━━━━━━━━━━━━━━━━◇
...
echo -e "$COLOR1 ${NC} ${WH}Port XHTTP   ${COLOR1}: ${WH}443" | tee -a /etc/vless/akun/log-create-${user}.log
...
echo -e "$COLOR1 ${NC} ${WH}Path         ${COLOR1}: ${WH}/vlessws" | tee -a /etc/vless/akun/log-create-${user}.log
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ${NC} ${COLOR1}Link XHTTP TLS${WH}:${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}${xhttp1}${NC}"  | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ${NC} ${COLOR1}Link XHTTP NTLS${WH}:${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}${xhttp2}${NC}"  | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
